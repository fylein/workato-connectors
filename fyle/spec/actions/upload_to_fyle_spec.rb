...
RSpec.describe "actions/upload_to_fyle", :vcr do
  ...
  let(:action) { connector.actions.upload_to_fyle }

  describe "input_fields" do
    subject(:output_fields) { action.input_fields(settings) }
    let(:expected_input_fields) { JSON.parse(File.read("fixtures/actions/upload_to_fyle/input_fields.json")) }
  ...
  describe "output_fields" do
    ...
    let(:expected_output_fields) { JSON.parse(File.read("fixtures/actions/upload_to_fyle/output_fields.json")) }
