...
RSpec.describe "actions/get_list_of_expenses", :vcr do

  # Spec describes the most commons blocks of an action. Remove describes that you don't need.
  # Learn more: https://docs.workato.com/developing-connectors/sdk/cli/reference/rspec-commands.html

  subject(:output) { connector.actions.get_list_of_expenses(input) }
  ...
  let(:action) { connector.actions.get_list_of_expenses }

  describe "output_fields" do
    subject(:output_fields) { action.output_fields(settings) }
    let(:expected_output_fields) { JSON.parse(File.read("fixtures/actions/get_expenses/output_fields.json")) }
