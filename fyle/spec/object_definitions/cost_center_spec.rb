...
RSpec.describe "object_definition/cost_center", :vcr do
  ...
  let(:object_definition) { connector.object_definitions.cost_center }

  describe "fields" do
    subject(:schema_fields) { object_definition.fields(settings) }
    let(:expected_output) { JSON.parse(File.read("fixtures/object_definations/cost_center.json")) }
